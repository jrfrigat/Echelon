...
internal static class GitLabUrls
{
    ...
    internal static Uri Branches(Uri apiUrl, string projectPath, int page = 1) =>
        new($"{Root(apiUrl)}/api/v4/projects/{Uri.EscapeDataString(projectPath)}/repository/branches?per_page=100&page={page}");
    ...
    internal static Uri Pipelines(Uri apiUrl, string projectPath, int perPage) =>
        new($"{Root(apiUrl)}/api/v4/projects/{Uri.EscapeDataString(projectPath)}/pipelines?per_page={perPage}&order_by=id&sort=desc");
