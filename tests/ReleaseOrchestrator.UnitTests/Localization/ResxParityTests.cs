...
namespace ReleaseOrchestrator.UnitTests.Localization;
...
public class ResxParityTests
{
    ...
    private static string LocateResourcesDir()
    {
        ...
        while (dir is not null && !dir.EnumerateFiles("ReleaseOrchestrator.slnx").Any())
            dir = dir.Parent;

        Assert.True(dir is not null, "Could not locate the repository root (ReleaseOrchestrator.slnx) from the test assembly.");
        return Path.Combine(dir!.FullName, "src", "ReleaseOrchestrator.Pwa", "Resources");
    }
}
