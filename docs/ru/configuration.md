...
читает его по таймеру, а не принимает вебхуки. Интервал каждое соединение хранит в своих настройках;
эти глобальные параметры задают, работает ли опрос и как часто он просыпается - нижнюю границу для
интервала соединения.

Опрос трекера спрашивает у него, какие задачи открыты, **и** перечитывает те, что уже известны как
открытые. Первая половина - это единственный способ вообще найти работу: опрашиваемый трекер не шлёт
вебхуков, и создать задачу больше нечему. Для неё соединение должно сказать, где искать: задайте
**Queues to sweep** (или собственный **Search query**) в настройках соединения, иначе опрос сообщит,
что искать не смог, и лишь обновит уже имеющееся. Вторая половина замечает обратный случай - задачу,
...
- **Что:** Сколько задач за проход ставится в очередь на синхронизацию по одному соединению трекера - суммарно по открытым в трекере и уже известным
- **Пример:** `500`
- **По умолчанию:** 500
