...
- **Пример:** `guest`
- **При отсутствии:** MassTransit падает при старте шины
...
- **При отсутствии:** MassTransit падает при подключении
...
- **При отсутствии:** Ошибка DNS, MassTransit падает
...
MassTransit consumers работают конкурентно. Настройте через окружение:
```bash
# Только на уровне кода (в Program.cs в текущий момент — PRs welcome)
// x.Concurrent = 10; // установить в MassTransit config
```
