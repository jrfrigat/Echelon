...
- **Примечание:** Регистрируется в каждой реплике, но работает под lease - за ночь цикл проходит один раз на весь деплой

**`Archiving__RunAtUtcHour`**
- **Что:** Час суток (UTC), в который стартует ночной цикл
- **Пример:** `2`
- **По умолчанию:** 2
- **Примечание:** Именно час, а не интервал - цикл идёт раз в ночь. Cron-выражения нет: без парсера в этой сборке его нельзя соблюсти, а настройка, которая читается, но игнорируется, хуже отсутствующей

**`Archiving__ArchiveAfterDays`**
- **Что:** Сколько MR или задача должны пробыть в терминальном состоянии, прежде чем станут пригодны к переносу
- **Пример:** `90`
- **По умолчанию:** 90
- **Поведение:** Отсчёт от времени слияния, закрытия MR или закрытия задачи - никогда от первого появления. Строка, на которую всё ещё ссылается план, раскатка или состояние деплоя, ждёт независимо от возраста

**`Archiving__MrBatchSize`**
- **Что:** Сколько MR переносится за один батч
- **Пример:** `500`
- **По умолчанию:** 500

**`Archiving__TaskBatchSize`**
- **Что:** Сколько задач переносится за один батч
- **Пример:** `1000`
- **По умолчанию:** 1000
- **Примечание:** Большие батчи = меньше раундов в БД, но более высокая lock contention

**`Archiving__StatusJournalRetentionDays`**
- **Что:** Сколько дней хранятся переходы статусов и лейблов MR
- **Пример:** `730`
- **По умолчанию:** 730 (два года)
- **Поведение:** Эти строки удаляются, а не архивируются - их читает только таймлайн задачи. Долгоживущая задача может потерять свои ранние переходы, оставаясь открытой; поднимайте значение, если эта история важнее объёма

**`Archiving__RolloutHistoryRetentionDays`**
- **Что:** Сколько дней хранится завершённая выкатка со своими шагами
- **Пример:** `730`
- **По умолчанию:** 730 (два года)
- **Поведение:** Удаляются только завершённые выкатки; незавершённая остаётся в любом возрасте. Шаги и события уходят по каскаду
- **Почему это важно:** Это не про порядок в таблицах. `RolloutStep` ссылается на задачу и MR через `Restrict`, поэтому пока история не устареет, выкаченную задачу вообще нельзя заархивировать - именно это даёт архиву сливаться

**`Archiving__PlanHistoryRetentionDays`**
- **Что:** Сколько дней хранится вытесненная версия плана
- **Пример:** `90`
- **По умолчанию:** 90
- **Поведение:** Также удаляет все версии задачи, закрытой раньше `ArchiveAfterDays`, включая активную - иначе `PlanTaskNode` держит задачу. Пересчёт строит план заново из атласа, так что ничего невосстановимого не теряется
- **Почему меньше истории выкаток:** Каждое событие приёма перестраивает все активные планы, то есть это шум, а не свидетельство - свидетельство это выкатка
...
- **Зачем нужно:** Трекеры не шлют событие при добавлении или удалении связи, поэтому без этого прохода добавленное в трекере ребро не появится, пока задачу не тронет что-то постороннее

**`TaskReconciliation__IntervalMinutes`**
- **Что:** Как часто синхронизировать открытые задачи
- **Пример:** `30`
- **По умолчанию:** 30
- **Примечание:** Работает под lease - за интервал проходит один проход на весь деплой, а не по одному на реплику

**`TaskReconciliation__MaxTasksPerRun`**
- **Что:** Сколько открытых задач перечитывается за проход, чтобы бэклог не завалил API трекера
- **Пример:** `500`
- **По умолчанию:** 500
- **Поведение:** Проход продолжается с того места, где остановился предыдущий, и заворачивается по кругу: если открытых задач больше лимита, это замедляет охват, но не оставляет задачи неохваченными навсегда
